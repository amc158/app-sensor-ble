...
export class BleService {
  ...
  async conectarESP32() {
    try {
      ...
      // Forzamos la máxima frecuencia de paquetes permitida por el SO
      try {
        await BleClient.requestConnectionPriority(device.deviceId, 'high' as any);
      } catch (e) {
        console.warn('Prioridad alta no soportada por el SO, usando default');
      }
  ...
  // --- OPTIMIZACIÓN: ENVÍO OTA DE ALTA VELOCIDAD ---
  async enviarOTA(firmware: ArrayBuffer) {
    ...
    try {
      ...
      // OPTIMIZACIÓN: Ajustado a 253 (MTU 256 - 3 bytes overhead)
      const CHUNK_SIZE = 253; 
      const view = new Uint8Array(firmware);
      const totalChunks = Math.ceil(view.length / CHUNK_SIZE);

      // Bucle de alta velocidad (sin esperas)
      for (let i = 0; i < totalChunks; i++) {
        ...
        // Menos actualizaciones de UI para ahorrar ciclos de CPU (cada 100 paquetes)
